...
import {
  ...
  productPurchases as mockPurchases,
} from "@/lib/mock-data";
...
interface InventoryStore {
  ...
  /** Applies consumption for a paid invoice; idempotent if invoice already has inventoryDeductedAt. */
...
export const useInventoryStore = create<InventoryStore>()(
  persist(
    (set, get) => ({
      ...
      recordStockAdjustment: (input) => {
        ...
        set((state) => {
          ...
          const unit =
            amountMl != null ? "ML" : p?.primaryUnit ?? "Piece";
          const movement: StockMovement = {
            id: `sm-${Date.now()}`,
            partId,
            type: direction,
    ...
    { name: "prime-detailers-inventory" }
  )
);
