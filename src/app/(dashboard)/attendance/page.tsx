...
import { attendanceRecords, staff } from "@/lib/mock-data";
...
export default function AttendancePage() {
  ...
  const absenceAlerts = useMemo(() => {
    ...
    return staff.filter((s) => {
      ...
      if (!record) return true; // No record = not checked in
      if (record.status === "ABSENT") return true;
      if (!record.checkIn) return true; // No check-in yet
      if (record.checkIn > cutoff) return true; // Checked in late
      return false;
    });
  }, [todayRecords]);
  ...
  const staffSummary = useMemo(() => {
    ...
    const periodRecords = attendanceRecords.filter(
      (r) => r.date <= selectedDate && r.date >= startStr
    );
    return staff.map((s) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <PageHeader
        ...
        description="QR-based check-in/check-out system"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-center w-full aspect-square max-w-[200px] mx-auto border-2 border-dashed border-muted-foreground/30 rounded-lg bg-muted/30">
              <QrCode className="w-16 h-16 text-muted-foreground" />
            </div>
            <p className="text-xs text-center text-muted-foreground">
              Scan QR code at the branch to check in/out
            </p>
            <div className="flex flex-col gap-2">
              <Button onClick={handleSimulateCheckIn} className="w-full">
                Simulate Check-In
              </Button>
              <Button
